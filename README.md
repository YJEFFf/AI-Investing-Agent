...
[장 시작]
현재가 조회 → half-Kelly × confidence 포지션 사이징 → 시가 매수
        ↓
...
| `scheduler.py` | APScheduler — 장전/개장/마감 스케줄링 (KST) |
| `notifier.py` | 텔레그램 매수/매도/요약 알림 |

### 리스크 (`risk/`)
| 파일 | 역할 |
|------|------|
| `position_sizer.py` | half-Kelly × confidence 포지션 사이징 (최대 25% 캡) |
...
**half-Kelly × confidence** 공식으로 종목당 매수 비율 결정

```
kelly = (승률 / 평균손실률) - ((1 - 승률) / 평균이익률)
half_kelly = kelly / 2
fraction = min(half_kelly, 0.25) × confidence × 0.88   # 12% 갭 버퍼
budget = 가용잔고 × fraction
```
...
- 이력 없을 때 기본값: 승률 50%, 평균이익 7%, 평균손실 5%
- half-Kelly가 25%를 초과하면 25% 캡 적용
- 매수마다 잔고가 차감되므로 후순위 종목 예산도 자동 감소
