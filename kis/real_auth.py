...
_TOKEN_CACHE_FILE = Path(__file__).parent.parent / "data_store" / "real_token_cache.json"
...
class KISRealAuth:
    ...
    async def _issue_token(self) -> None:
        url = f"{settings.kis_real_base_url}/oauth2/tokenP"
        payload = {
    ...
    async def get_hashkey(self, body: dict) -> str:
        ...
        url = f"{settings.kis_real_base_url}/uapi/hashkey"
        headers = {
