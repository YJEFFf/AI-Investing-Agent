...
from kis.real_auth import kis_real_auth
...
class KISRealClient:
    ...
    async def post(self, path: str, tr_id: str, body: dict) -> dict:
        hashkey = await kis_real_auth.get_hashkey(body)
    ...
    async def _request(
        ...
        token = await kis_real_auth.get_token()
        headers = {
        ...
        url = f"{settings.kis_real_base_url}{path}"
        ...
        raise RuntimeError(f"KIS real API request failed after {_MAX_RETRIES} retries")

    async def close(self):
        await self._client.aclose()


kis_real_client = KISRealClient()
